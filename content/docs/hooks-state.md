...
* **Строка 9:** Когда пользователь кликает по кнопке, мы вызываем анонимную функцию которой присвоили `setCount` с инкрементирующим значением. После этого React сделает повторный рендер, в котором использует уже новое значение `count`.
